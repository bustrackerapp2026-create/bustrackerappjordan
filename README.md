...
### قاعدة ملكية المسار التشغيلي للمركبة — 2026-09-26

تم تثبيت القرار المعماري التالي قبل استكمال Failure Paths:

- **المركبة هي مصدر الحقيقة للمسار التشغيلي المعتمد.**
- `busNumber` يحدد المركبة التشغيلية في البنية الحالية، بينما `routeId` يحدد المسار المعتمد الذي تعمل عليه المركبة.
- `driverId` يحدد السائق الذي يقود/يستخدم المركبة حاليًا لأغراض الهوية والأمان والتدقيق، وليس مصدرًا لملكية المسار.
- لا ينبغي أن يؤدي تغيير السائق الذي يقود نفس المركبة إلى تغيير المسار التشغيلي للمركبة.
- يوجد حاليًا fallback في `startTrip()` يسمح بالبحث عن تعيين مرتبط بالسائق عند عدم العثور على تعيين للمركبة؛ **هذا السلوك سيتم مراجعته وإزالته أو تضييق نطاقه في تغيير مستقل لاحقًا** حتى يصبح التشغيل النهائي معتمدًا على تعيين المركبة فقط، مع اختبار Failure/Recovery خاص به.
